function [multi] = TargetTracking(scenario)
...
%TODO: REMOVE OUTDATED FLAG
switch 'Kalman Filter'
    
    case 'Moving Average'
        % Moving Average Filter Smoothing
        for dim = 1:2
            multi.track_points(dim,:) = movmean(multi.lat_points(dim,:), radarsetup.wind_size(dim));
        end
...
%TODO: REMOVE OUTDATED FLAG
        ...
        T = radarsetup.n_p*radarsetup.cpi_fr/radarsetup.prf;
        
        % Set up variance values
        sig_v = multi.tracking.sigma_v(1);
        sig_z = multi.tracking.sigma_z(1);
