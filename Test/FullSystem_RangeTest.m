        ...
        for range_ind = 1:length(range_list)
            ...
            for iter = 1:iterations
                ...
                if det_list.num_detect > 0
                    ...
                    range_true(range_ind, ang_ind, rcs_ind, iter) = range_in;
                    ang_true(range_ind, ang_ind, rcs_ind, iter) = ang_in;
                    vel_true(range_ind, ang_ind, rcs_ind, iter) = vel_in;
                    
                    range_meas(range_ind, ang_ind, rcs_ind, iter) = det_list.range(max_ind);
                    ang_meas(range_ind, ang_ind, rcs_ind, iter) = det_list.az(max_ind);
                    vel_meas(range_ind, ang_ind, rcs_ind, iter) = det_list.vel(max_ind);
                    
                    range_err(range_ind, ang_ind, rcs_ind, iter) = det_list.range(max_ind) - range_in;
                    ang_err(range_ind, ang_ind, rcs_ind, iter) = det_list.az(max_ind) - ang_in;
                    vel_err(range_ind, ang_ind, rcs_ind, iter) = det_list.vel(max_ind) - vel_in;
                    snr_meas(range_ind, ang_ind, rcs_ind, iter) = det_list.SNR(max_ind);
                    snr_calc(range_ind, ang_ind, rcs_ind, iter) = CalculateSNR(scenario);
                end
            ...
            save(filename, ...
                ...
                'iterations', 'range_ind', 'ang_ind', 'rcs_ind');
